...
class LoginForm:
    def __init__(self):
        ...
        self.background_img = self.background_img.resize((self.wn.winfo_screenwidth(), self.wn.winfo_screenheight()))
        ...
        # ================ LABELS AND ENTRIES =====================
        self.canvas.create_text(self.wn.winfo_screenwidth() // 2, 150, text="Login", fill='black', font=('times', 60, 'bold'))
        
        self.canvas.create_text(self.wn.winfo_screenwidth() // 2 - 150, 300, text="Username", fill='black', font=('times', 30, 'bold'))
        self.canvas.create_text(self.wn.winfo_screenwidth() // 2 - 150, 400, text="Password", fill='black', font=('times', 30, 'bold'))

        # Username Entry
        self.Username = Entry(self.wn, font=('bell mt', 20, 'bold'), textvariable=self.Username_value)
        self.Username.place(x=self.wn.winfo_screenwidth() // 2, y=285, width=300, height=40)

        # Password Entry
        self.Password_Show = Entry(self.wn, font=('bell mt', 20, 'bold'), textvariable=self.Password_value, show="*")
        self.Password_Show.place(x=self.wn.winfo_screenwidth() // 2, y=385, width=300, height=40)

        # Show/Hide Password Button
        self.hide_show_btn = Button(self.wn, text="Show", font=('bell mt', 14, 'bold'), height=1, width=6,
                                    command=self.toggle_password, bd=0)
        self.hide_show_btn.place(x=self.wn.winfo_screenwidth() // 2 + 320, y=385)
        ...
        self.Login.place(x=self.wn.winfo_screenwidth() // 2 - 50, y=480)
